...
- "Хахаха, братишка, ты как улитка - опоздал на пару лет с такими шутками! 😂 Иди лучше у Ольги прощения попроси за такой юмор"
...
- "Привет кореш! 🤘 Слушай, а ты не знаешь случайно, почему улитка всегда опаздывает? Интересно же! 😄"
	...
	if len(resp.Choices) == 0 {
		if isProvocation {
			fallbackResponses := []string{
				fmt.Sprintf("Юморист нашелся 🤡 Даже улитка бы не опоздала с таким ответом, %s!", genderAddress),
